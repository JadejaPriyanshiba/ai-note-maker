...
- **Follow the existing view-router pattern** — the `AppView` string union and `ROUTES` table in `src/lib/router.ts`, with `activeView` in `App.tsx` derived from the URL — rather than introducing React Router or any other routing library. Navigate with `go(view, params)`, never by reintroducing `setActiveView` state. Any path carrying an entity id also needs a branch in `App.tsx`'s deep-link hydration effect, or the URL breaks on reload. See [DEVELOPMENT.md](docs/DEVELOPMENT.md#adding-a-new-view).
